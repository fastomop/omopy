        ...
        for ing_id, ing_name in zip(ingredient_ids, ingredient_names, strict=False):
            ...
            if check == "missing":
                ...
                for qn, qv in zip(
                    _QUANTILE_NAMES,
                    [
                        max(1, mean_dur - 2 * sd_dur),
                        max(1, mean_dur - 1.5 * sd_dur),
                        max(1, mean_dur - 0.7 * sd_dur),
                        mean_dur,
                        mean_dur + 0.7 * sd_dur,
                        mean_dur + 1.5 * sd_dur,
                        mean_dur + 2 * sd_dur,
                ...
                for qn, qv in zip(
                    _QUANTILE_NAMES,
                    [
                        max(1, mean_ds - 2 * sd_ds),
                        max(1, mean_ds - 1.5 * sd_ds),
                        max(1, mean_ds - 0.7 * sd_ds),
                        mean_ds,
                        mean_ds + 0.7 * sd_ds,
                        mean_ds + 1.5 * sd_ds,
                        mean_ds + 2 * sd_ds,
                ...
                row["n_days_supply_or_dates_missing"] = (
                    n - n_match - row["n_days_supply_differ_date_diff"]
                )
                parts.append(pl.DataFrame([row]))
                ...
                for qn, qv in zip(
                    _QUANTILE_NAMES,
                    [
                        max(0, mean_qty - 2 * sd_qty),
                        max(0, mean_qty - 1.5 * sd_qty),
                        max(0, mean_qty - 0.7 * sd_qty),
                        mean_qty,
                        mean_qty + 0.7 * sd_qty,
                        mean_qty + 1.5 * sd_qty,
                        mean_qty + 2 * sd_qty,
                ...
                for qn, qv in zip(
                    _QUANTILE_NAMES,
                    [
                        max(0, mean_gap - 2 * sd_gap),
                        max(0, mean_gap - 1.5 * sd_gap),
                        max(0, mean_gap - 0.7 * sd_gap),
                        mean_gap,
                        mean_gap + 0.7 * sd_gap,
                        mean_gap + 1.5 * sd_gap,
                        mean_gap + 2 * sd_gap,
